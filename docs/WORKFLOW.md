...
- **Models stay pinned.** Planning on `nemotron-3-ultra-free` (m3j-planner), gates/review on `laguna-s-2.1-free` (m3j-sentinel) and `longcat-2.0-free` (m3j-triage), execution on `deepseek-v4-flash-free` (m3j-executor, m3j-devops), tests on `north-mini-code-free` (m3j-tester). Don't override.
