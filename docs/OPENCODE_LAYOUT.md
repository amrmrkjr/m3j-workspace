...
└── site/                        # static reference guide — no build step
    ├── index.html · setup.html · reference.html
    ├── agents.html · workflow.html           (5 pages, shared nav)
...
|---|---|---|
| `opencode.json` | Primary OpenCode config | Model `opencode/deepseek-v4-flash-free`; plan/compaction agents `opencode/deepseek-v4-flash-free`; skills path `.agents/skills`; instructions `AGENTS.md`. Edit only through `customize-opencode` skill. |
...
| `site/` | Static reference guide | 5 HTML pages sharing one nav partial (home → setup → reference → agents → workflow). Design language: "amber signal / carbon" (hairline panels, amber mono). Adding/removing a page = update every nav + homepage card grid + `README.md`. |
